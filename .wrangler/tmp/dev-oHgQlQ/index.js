...
var worker_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname.startsWith("/api/")) {
      return handleApiRoutes(request, env2, ctx);
    }
    try {
      ...
      console.error("Error fetching asset:", error3);
    }
    if (!url.pathname.includes(".")) {
      try {
        ...
        console.error("Error serving index.html:", error3);
      }
    }
    return new Response("Not Found", { status: 404 });
  }
};
async function handleApiRoutes(request, env2, _ctx) {
  const url = new URL(request.url);
  ...
  if (url.pathname === "/api/generate-comm-template" && request.method === "POST") {
    return handleGenerateCommTemplate(request, env2);
  }
  return new Response("API endpoint not found", { status: 404 });
}
__name(handleApiRoutes, "handleApiRoutes");
async function handleGenerateCommTemplate(request, env2) {
  try {
    const body = await request.json();
    const { taskText, context: context2 } = body;
    if (!taskText) {
...
${context2 ? `Additional context: ${context2}` : ""}
...
Template:`;
    ...
    console.error("Error generating template:", error3);
    return new Response(JSON.stringify({
...
__name(handleGenerateCommTemplate, "handleGenerateCommTemplate");
...
// .wrangler/tmp/bundle-3ivEi3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3ivEi3/middleware-loader.entry.ts
